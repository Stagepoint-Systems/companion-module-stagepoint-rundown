...
/**
 * Send an authenticated POST to a Companion API endpoint on the Rundown host.
 * Updates module status based on the response so connection issues surface in Companion.
 */
	...
	return {
		...
		cue_adjust: {
			...
			callback: async (event, context) => {
				const raw = await context.parseVariablesInString(event.options.deltaSeconds)
				const delta = parseInt(raw, 10)
				if (!Number.isFinite(delta) || delta === 0) {
					self.log('warn', `Adjust Time: invalid delta "${raw}"`)
					return
				}
				await callApi(self, '/api/companion/cue/adjust', { deltaSeconds: delta })
			},
		},
		...
		message_bg_color: {
			...
			callback: async (event) => {
				await callApi(self, '/api/companion/message/bg-color', { color: event.options.color })
			},
		},
